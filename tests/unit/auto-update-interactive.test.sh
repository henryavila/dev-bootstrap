...
# the interactive-menu gate's perspective).
{
    ...
    printf 'INCLUDE_IDENTITY=%s\n' "\${INCLUDE_IDENTITY:-}"
    printf 'INCLUDE_NPM_GLOBAL=%s\n' "\${INCLUDE_NPM_GLOBAL:-}"
    printf 'MESH_NPM_GLOBAL=%s\n' "\${MESH_NPM_GLOBAL:-}"
    printf 'MESH_AI_PACKAGES=%s\n' "\${MESH_AI_PACKAGES:-}"
    printf 'DRY_RUN=%s\n' "\${DRY_RUN:-}"
...
export INCLUDE_POSTGRES=1 PHP_VERSIONS=8.3 PHP_DEFAULT=8.3 POSTGRES_VERSION=16 MESH_IDENTITY_REPO=git@github.com:foo/bar
# CP4 D-F-003: newer gates that should ALSO be wiped.
export INCLUDE_AI_TOOLS=1 INCLUDE_CODE_SERVER=1 INCLUDE_IDENTITY=1 INCLUDE_NPM_GLOBAL=1
export MESH_NPM_GLOBAL=1 MESH_AI_PACKAGES=1
...
for v in NON_INTERACTIVE CI ONLY_TOPICS DRY_RUN INCLUDE_DOCKER INCLUDE_WEBSTACK \
         ...
         PHP_VERSIONS PHP_DEFAULT POSTGRES_VERSION MESH_IDENTITY_REPO \
         INCLUDE_AI_TOOLS INCLUDE_CODE_SERVER INCLUDE_IDENTITY \
         INCLUDE_NPM_GLOBAL MESH_NPM_GLOBAL MESH_AI_PACKAGES; do
