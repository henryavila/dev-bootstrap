...
FAKE=$TMP/home
mkdir -p "$FAKE/.local/state/mesh-workstation/snapshots/$(hostname)-pre-migration"
SNAP="$FAKE/.local/state/mesh-workstation/snapshots/$(hostname)-pre-migration"
...
# Lock file
touch "$FAKE/.local/state/mesh-workstation/migration.lock"
...
if [[ ! -f "$FAKE/.local/state/mesh-workstation/migration.lock" ]]; then passed=$((passed+1)); echo "  ✓ lock released"
...
FAKE2=$TMP/home2
mkdir -p "$FAKE2/.local/state/mesh-workstation/snapshots/$(hostname)-pre-migration"
...
touch "$FAKE2/.local/state/mesh-workstation/migration.lock"
...
if [[ -f "$FAKE2/.local/state/mesh-workstation/migration.lock" ]]; then
...
fixture_with_metadata() {
    local fake="$1"
    mkdir -p "$fake/.local/state/mesh-workstation/snapshots/$(hostname)-pre-migration"
    local snap="$fake/.local/state/mesh-workstation/snapshots/$(hostname)-pre-migration"
...
printf 'pid=%s\nhost=%s\nstarted=2026-05-20T00:00:00Z\n' "$dead_pid" "$(hostname)" \
    > "$FAKE_DEADPID/.local/state/mesh-workstation/migration.lock"
HOME=$FAKE_DEADPID bash "$FAKE/repo/scripts/migrate-rollback.sh" > /tmp/rollback-deadpid.out 2>&1
deadpid_rc=$?
if [[ $deadpid_rc -eq 0 ]] && [[ ! -f "$FAKE_DEADPID/.local/state/mesh-workstation/migration.lock" ]]; then
    passed=$((passed+1)); echo "  ✓ F-005: dead pid → lock cleared, rollback succeeds"
else
    failed=$((failed+1)); echo "  ✗ F-005: dead pid scenario (rc=$deadpid_rc, lock present=$([ -f "$FAKE_DEADPID/.local/state/mesh-workstation/migration.lock" ] && echo yes || echo no))" >&2
fi
...
printf 'pid=1\nhost=not-this-host-%s\nstarted=2026-05-20T00:00:00Z\n' "$$" \
    > "$FAKE_OTHERHOST/.local/state/mesh-workstation/migration.lock"
HOME=$FAKE_OTHERHOST bash "$FAKE/repo/scripts/migrate-rollback.sh" > /tmp/rollback-otherhost.out 2>&1
otherhost_rc=$?
if [[ $otherhost_rc -eq 0 ]] && [[ ! -f "$FAKE_OTHERHOST/.local/state/mesh-workstation/migration.lock" ]]; then
...
printf 'pid=%s\nhost=%s\nstarted=2026-05-20T00:00:00Z\n' "$$" "$(hostname)" \
    > "$FAKE_ACTIVE/.local/state/mesh-workstation/migration.lock"
active_rc=0
HOME=$FAKE_ACTIVE bash "$FAKE/repo/scripts/migrate-rollback.sh" > /tmp/rollback-active.out 2>&1 || active_rc=$?
if [[ $active_rc -eq 2 ]] && [[ -f "$FAKE_ACTIVE/.local/state/mesh-workstation/migration.lock" ]]; then
    passed=$((passed+1)); echo "  ✓ F-005: active pid → rollback exits 2, lock preserved"
else
    failed=$((failed+1)); echo "  ✗ F-005: active-pid scenario (rc=$active_rc, lock present=$([ -f "$FAKE_ACTIVE/.local/state/mesh-workstation/migration.lock" ] && echo yes || echo no))" >&2
fi
...
force_rc=$?
if [[ $force_rc -eq 0 ]] && [[ ! -f "$FAKE_ACTIVE/.local/state/mesh-workstation/migration.lock" ]]; then
