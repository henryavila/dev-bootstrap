...
TARGETS=(
    "$ROOT/topics/languages/wsl/php-stack.sh"
    "$ROOT/topics/web/wsl/packages.sh"
    "$ROOT/topics/databases/scripts/install-mssql-driver.sh"
)
...
if grep -qE '(no-install-recommends|libapache2-mod-php)' "$ROOT/topics/languages/wsl/php-stack.sh"; then
    pass "languages/wsl/php-stack.sh: justifies --no-install-recommends in a comment"
else
