#!/usr/bin/env bash
# tests/integration/ai-pinned.test.sh
#
# Contract suite for `mesh ai` pinned-projects layer (ad-hoc mesh-ai-pinned).
#
# Covers the source-only reader ai-pinned.sh (manifest parse, alternate-path
# pick, skip-missing, ~ expansion, non-git allowed, comments/blanks) and the
# runner verbs add/remove/list + the merged ai_catalogue (pinned-wins dedup),
...
# shellcheck source=../../scripts/lib/ai-pinned.sh
source "$REPO_ROOT/scripts/lib/ai-pinned.sh"
...
SANDBOX="$(mktemp -d -t mesh-ai-pin.XXXXXX)"
...
# path at the sandbox. Verbs only touch $MESH_AI_PINNED; --list also takes AI_ROOTS.
run_verb() { HOME="$HOME_SANDBOX" MESH_AI_PINNED="$PINS" MESH_IDENTITY_DIR="$SANDBOX" bash "$RUNNER" "$@"; }
run_verb_prompt() {
    local input="$1"; shift
...
# ── ai_pinned: parse, alternates, skip-missing, ~, comments, non-git ─────────
echo "── ai_pinned ──"
...
EOF
OUT="$(HOME="$HOME_SANDBOX" MESH_AI_PINNED="$PINS" ai_pinned)"
...
ASSERT_MSG="missing manifest is not an error" assert_exit_code 0 'MESH_AI_PINNED="'"$SANDBOX"'/absent.list" ai_pinned'
# Non-git dir is allowed (pinned overrides ai_discover's .git gate).
NGFILE="$SANDBOX/ng.list"; printf 'notes|%s\n' "$SANDBOX/notgit" > "$NGFILE"
assert_contains "$(MESH_AI_PINNED="$NGFILE" ai_pinned)" $'notes\t'"$SANDBOX/notgit" "pins a non-git dir"

# ── ai_catalogue (via --list): merge + pinned-wins + substring ───────────────
echo "── ai_catalogue (via --list) ──"
...
EOF
LIST_OUT="$(HOME="$HOME_SANDBOX" MESH_AI_PINNED="$PINS" MESH_IDENTITY_DIR="$SANDBOX" AI_ROOTS="$R1" bash "$RUNNER" --list)"
...
SUB="$(HOME="$HOME_SANDBOX" MESH_AI_PINNED="$PINS" MESH_IDENTITY_DIR="$SANDBOX" AI_ROOTS="$R1" bash "$RUNNER" --list onlypinned)"
...
# ── mesh ai add: create, idempotent update, default name, missing reject ────
echo "── mesh ai add ──"
rm -f "$PINS"
...
echo "── mesh ai add: git persistence prompt ──"
...
printf '# pins\n' > "$GITROOT/shell/ai-pinned.list"
git -C "$GITROOT" add shell/ai-pinned.list
...
PINS="$GITROOT/shell/ai-pinned.list"
...
ASSERT_MSG="accepted add reaches the remote" assert_true "git -C '$REMOTE' show main:shell/ai-pinned.list | grep -q '^gitproj|'"
...
assert_contains "$ADD_LOCAL_OUT" "git -C '$GITROOT_REAL' add 'shell/ai-pinned.list'" "declined add prints the exact recovery command"
PINS="$SANDBOX/pins.list"

# ── mesh ai remove: by name, unknown ────────────────────────────────────────
echo "── mesh ai remove ──"
...
echo "── mesh ai remove: git persistence prompt ──"
...
printf 'remove-me|%s\nkeep-me|%s\n' "$SANDBOX/projA" "$SANDBOX/projA2" > "$GITROOT_RM/shell/ai-pinned.list"
git -C "$GITROOT_RM" add shell/ai-pinned.list
git -C "$GITROOT_RM" commit -qm "init remove pins"
git -C "$GITROOT_RM" branch -M main
git -C "$GITROOT_RM" push -q -u origin main
PINS="$GITROOT_RM/shell/ai-pinned.list"
...
ASSERT_MSG="accepted remove reaches the remote" assert_false "git -C '$REMOTE_RM' show main:shell/ai-pinned.list | grep -q '^remove-me|'"
ASSERT_MSG="accepted remove preserves other remote pins" assert_true "git -C '$REMOTE_RM' show main:shell/ai-pinned.list | grep -q '^keep-me|'"
PINS="$SANDBOX/pins.list"
