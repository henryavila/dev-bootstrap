...
# scripts/lib/ia-herdr.sh — herdr handoff for `mesh ia`.
# Source-only (no top-level execution).
#
# Once a project is chosen, `mesh ia` does NOT multiplex or track state itself —
...
ia_in_herdr() { [[ -n "${HERDR_ENV:-}" ]]; }

# rc0 iff the herdr CLI + jq are both available; logs which is missing.
ia_herdr_ready() {
    if ! command -v herdr >/dev/null 2>&1; then
        log_error "ia: herdr not found on PATH — install herdr (https://herdr.dev) or run the agent directly"
        return 1
    fi
    if ! command -v jq >/dev/null 2>&1; then
        log_error "ia: jq not found on PATH — required to parse herdr JSON"
        return 1
    fi
    return 0
}
...
ia_herdr_workspace_id() {
    local name="$1"
    ia_herdr_workspaces | awk -F'\t' -v n="$name" '$1 == n { print $2; exit }'
}
...
# is where an agent actually lives, so this is what lets `mesh ia` jump to a
...
ia_herdr_open() {
    ...
    log_info "ia: creating workspace '$name' at $path"
    ...
    if [[ -z "$pane_id" ]]; then
        log_error "ia: herdr workspace create failed for '$name' ($path)"
        return 1
    fi
