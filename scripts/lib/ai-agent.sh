...
# scripts/lib/ai-agent.sh — agent command resolution + per-project memory for `mesh ai`.
...
# safe bare default):
#   MESH_AI_AGENT          default agent name (identity → 'claude'); fallback 'claude'
#   MESH_AI_FLAGS_<AGENT>  extra flags per agent, name uppercased and non-alnum→'_'
#                          (identity → MESH_AI_FLAGS_CLAUDE='--dangerously-skip-permissions')
#
# The last agent used for a project is remembered in a tiny state file, so
# `mesh ai <proj>` reopens with the same agent and never asks; `--agent X`
# overrides for this run AND updates the memory.

# Path to the per-project agent memory (KEY=value lines, project=agent).
ai_agent_state_file() {
    printf '%s/mesh/ai-agents.env' "${XDG_STATE_HOME:-$HOME/.local/state}"
}

# Print the remembered agent for a project name, or nothing.
ai_agent_get() {
    local proj="$1" f key val
    f="$(ai_agent_state_file)"
    [[ -r "$f" ]] || return 0
...
# MESH_AI_FLAGS_<AGENT>, empty when unset → a bare `claude`/`codex` for forks).
ai_agent_cmd() {
    local agent="$1" var flags
    var="MESH_AI_FLAGS_$(printf '%s' "$agent" | tr '[:lower:]' '[:upper:]' | tr -c 'A-Z0-9' '_')"
    var="${var%_}"
    flags="${!var:-}"
    if [[ -n "$flags" ]]; then
