...
SKIP_EXACT=(
    ...
    "ssh/authorized_keys"
    "ssh/config"
    # .gitattributes is tooling-generated by `mesh secret init` (the git-crypt
    # managed block), not a copy-the-scaffold file — so it needs no .example.
    ".gitattributes"
    # A git worktree's root .git is a FILE (a gitdir pointer), not the .git/
    # directory the SKIP_PREFIXES ".git/" entry covers. When template-check runs
    # with MESH_IDENTITY_DIR pointing at an in-repo worktree (.worktrees/<name>)
    # — as the pre-commit hook does via `git rev-parse --show-toplevel` — that
    # .git file would otherwise be read as un-templated identity content. Skip it.
    ".git"
)
